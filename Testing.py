...
while (True):
    ...
    detectionFrame = frame[int(videoHeight/2):int(realHeight)-int(videoHeight/2), int(videoWidth/2):int(realWidth)-int(videoWidth/2), :]
    ...
    frame[int(videoHeight/2):int(realHeight)-int(videoHeight/2), int(videoWidth/2):int(realWidth)-int(videoWidth/2), :] = outputFrame
    cv2.rectangle(frame, (int(videoWidth/2) , int(videoHeight/2)), (int(realWidth)-int(videoWidth/2), int(realHeight)-int(videoHeight/2)), boxColor, boxWeight)
    if i > bpmBufferSize:
        cv2.putText(frame, "BPM: %d" % bpmBuffer.mean(), bpmTextLocation, font, fontScale, fontColor, lineType)
        eric_output.append(bpmBuffer.mean())
    else:
...
with open('eric_test.csv', 'w', newline='') as file:
    wr = csv.writer(file, quoting=csv.QUOTE_ALL)
    for value in eric_output:
        wr.writerow([value])
if len(sys.argv) != 2:
